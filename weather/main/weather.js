...
async function main() {
  ...
  saveEditButton.addEventListener("click", () => {
    ...
    if (cityIndex !== -1) {
      ...
      cities[cityIndex].lat = parseFloat(document.getElementById("editLat").value).toFixed(3); // Round to 4 places
      cities[cityIndex].lon = parseFloat(document.getElementById("editLon").value).toFixed(3);// Round to 4 places
...
async function search() {
  ...
  if (response.ok) {
    let payload = await response.json(); // Store the result in the object using the URL as key;

    if (payload && payload.results && payload.results.length > 0) {
      for (const result of payload.results) {
        str += `<div onclick='load("${result.name}",${result.latitude},${result.longitude});  dialog.close();'>`;
        str += `${result.name}`;
...
function drawTable() {
  ...
  const dateOptions = {
    ...
    // timeZone: "Europe/London" // firstResult.timezone
  };
  ...
  for (const day of days) {
    ...
    // Month is 0-based in JavaScript Date object

    str += `<th>${new Date(iYear, iMonth - 1, iDay).toLocaleDateString(
      "en-US",
      dateOptions
    )}</th>`;
  }
  ...
  for (const city of cities) {
    ...
    if (city.from && city.to) {
      ...
      // Format the dates using the new helper function
      const formattedFrom = formatDate(city.from);
      const formattedTo = formatDate(city.to);

      str += `<br/>${formattedFrom} - ${formattedTo}`;
    }

    str += `<br/><button style='cursor:hand;' title='Edit' onclick='editCity("${city.id}")'>&#9998;</button>
    <button style='cursor:hand;' title='Remove' onclick='removeCity("${city.id}")'>x</button>`;


    let intOffset = cityData.utc_offset_seconds / 3600;
    let strOffset = intOffset < 0 ? intOffset : "+" + intOffset;
    str += `<br/>${cityData.timezone}`;
    str += `<br/>&#128336;${strOffset}h  &#9651;${cityData.elevation}m `;
    // inside drawTable(), modify this line:
    ...
    for (const day of days) {
      ...
      if (showDay) {
        ...
        str += `${Math.round(
          cityData.daily.apparent_temperature_max[dayCounter]
        )}&deg; / ${Math.round(cityData.daily.apparent_temperature_min[dayCounter])}&deg;`;
        ...
        let descr =
          weatherIcons[cityData.daily.weather_code[dayCounter]].day.description;
        str += `<br/><img src="${url}" width="40px"></img>`;
        str += `<br/>${descr}`;
    ...
    str += `<td>`
    // str += `${city.name}<br/>`;
    str += `${city.lat}<br/>${city.lon}</a>`;
    str += `<br/><a target="_blank" href="https://www.visualcrossing.com/weather-forecast/${city.lat},${city.lon}">WX 1 &#8599;</a>`;
    str += `<br/><a target="_blank" href="https://www.timeanddate.com/weather/@${city.lat},${city.lon}/ext">WX 2 &#8599;</a>`;
    str += `<br/><a target="_blank" href="https://forecast.weather.gov/MapClick.php?textField1=${city.lat}&textField2=${city.lon}">WX (US) &#8599;</a>`;
    // str += `<br/><a target="_blank" href="https://www.timeanddate.com/weather/@${city.lat},${city.lon}">T&D &#8599;</a>`
    // str += `<br/><a target="_blank" href="https://forecast.weather.gov/MapClick.php?textField1=${city.lat}&textField2=${city.lon}">W gov &#8599;</a>`
    // 34.142319&textField2=-118.704527
    // 34.142319,-118.704527
...
function editCity(cityId) {
  ...
  if (city) {
    ...
    document.getElementById("editLat").value = parseFloat(city.lat).toFixed(3);// Round to 4 places
    document.getElementById("editLon").value = parseFloat(city.lon).toFixed(3);// Round to 4 places
...
function initializeMap() {
  ...
  function onMapClick(e) {
    ...
    document.getElementById('mapLocation').textContent = "Selected location: " + e.latlng.toString();
...
function openMapDialog() {
  ...
  // Use a MutationObserver to detect when the dialog is added to the DOM
    ...
    for (const mutation of mutationsList) {
      if (mutation.type === 'attributes' && mutation.attributeName === 'open') {
        ...
        if (mapDialog.open) {
          ...
          // Disconnect the observer as we only need to run this once
          observer.disconnect();
          return; // Exit loop
        }
      }
  ...
  // Start observing the dialog for changes to its 'open' attribute
  observer.observe(mapDialog, { attributes: true });

  mapDialog.showModal();

  // Handle the "Use this Location" button
  const useMapLocationButton = document.getElementById('useMapLocation');
  useMapLocationButton.onclick = () => {
    // You can add any other logic here when you want to use the location
...
async function getNearestCity(lat, lon) {
  try {
    ...
    if (data.address) {
      ...
      document.getElementById('mapLocation').textContent = `Selected location: ${lat}, ${lon} (Nearest City: ${city})`;

      mapFoundCity = city;
      // console.log("CITY", city);

    } else {
      document.getElementById('mapLocation').textContent = `Selected location: ${lat}, ${lon} (No city found)`;
      // console.log('No address information found.');
    }
  } catch (error) {
    console.error('Error fetching nearest city:', error);
    document.getElementById('mapLocation').textContent = `Selected location: ${lat}, ${lon} (Error finding city)`;
  }
}
